...
const bodySchema = z.object({ paymentId: uuidSchema, reason: z.string().min(3).max(300), idempotencyKey: z.string().min(10).max(100) });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("reverse_payment", { p_payment_id: parsed.data.paymentId, p_reason: parsed.data.reason, p_idempotency_key: parsed.data.idempotencyKey });
    ...
    log("info", "payments.reversed", { workspaceId: user.workspaceId, paymentId: parsed.data.paymentId });
